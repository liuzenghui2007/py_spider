...
class HypeAuditorCategorySpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        self.file = open('hypeauditor_all_results.csv', 'w', newline='', encoding='utf-8')
        self.csv_writer = csv.writer(self.file)
        self.csv_writer.writerow(['rank', 'nick', 'firstName', 'category', 'followers', 'country', 'engAuth', 'engAvg', 'category_name'])
    ...
    def parse(self, response):
        ...
        for row in response.css('.table .row[data-v-40a1893f]'):
            ...
            self.csv_writer.writerow([
                item['rank'], item['nick'], item['firstName'], item['category'],
                item['followers'], item['country'], item['engAuth'], item['engAvg'], category
            ])
